...
public class Test_Pets {
	...
	{
		String myJson = "{ \"id\": \"777888999\", \"category\": { \"id\": 0, \"name\": \"string\" }, \"name\": \"doggie\", \"photoUrls\": [ \"string\" ], \"tags\": [ { \"id\": 0, \"name\": \"string\" } ], \"status\": \"available\"}";
		RestAssured.baseURI = "https://petstore.swagger.io/v2";

		Response r = given().
		// log().all().
				contentType("application/json").body(myJson).when().post("/pet");
...

	{
		...
		String myJson = "{ \"id\": \"" + petId
				+ "\", \"category\": { \"id\": 10, \"name\": \"rex\" }, \"name\": \"ziz\", \"photoUrls\": [ \"string\" ], \"tags\": [ { \"id\": 0, \"name\": \"string\" } ], \"status\": \"available\"}";
		RestAssured.baseURI = "https://petstore.swagger.io/v2";

		Response r = given().
		// log().all().
				contentType("application/json").body(myJson).when().put("/pet");
...

	{
		...
		Response r = given().
		// log().all().
				contentType("application/json").when().delete("/pet" + petId);
